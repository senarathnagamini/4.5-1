...
const fs = require('fs');
const Db = require('../RAVANA/plugin');
...
Raone.useCmd({pattern: 'insert ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN, dontAddCommandList: true}, (async (message, match) => {
    ...
    if (response.statusCode == 200) {
        ...
        try {
            ...
            fs.unlinkSync('/root/4.5/plugins' + plugin_name + '.js')
        ...
        if (response.body.includes('../RAVANA/notes') || response.body.includes('../RAVANA/plugin') || response.body.includes('../RAVANA/greetings') || response.body.includes('../RAVANA/filters')) DEG.level = DEG.level + 33
